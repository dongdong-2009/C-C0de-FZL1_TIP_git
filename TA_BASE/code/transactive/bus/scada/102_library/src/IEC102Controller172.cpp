...
{
	IEC102Controller172::IEC102Controller172(ChannelManager& channelManager,IEC102ConfigData& objConfigData,unsigned int startAddr, unsigned int endAddr, time_t startTime, time_t endTime, unsigned char rad)
	:IEC102FlowController(channelManager,objConfigData),
	 m_protocol172(NULL)
	{
		FUNCTION_ENTRY("IEC102Controller172()");
		m_protocol172 = new IEC102Protocol172(*this);
		m_102Protocol = m_protocol172;
	...
	IEC102Controller172::~IEC102Controller172()
	{
		FUNCTION_ENTRY("~IEC102Controller172()");
	...
	bool IEC102Controller172::processFlow()
	{
	...
	void IEC102Controller172::setFlowInfo()
	{
		...
		m_protocol172->setVirtualAddress(virtualAddr);
	...
	bool IEC102Controller172::getNextFlow()
	{
	...
	void IEC102Controller172::getClassIData()
	{
		FUNCTION_ENTRY("getClassIData()");
		std::string frameData = m_protocol172->getClassIData();
	...
	void IEC102Controller172::call172Data()
	{
		FUNCTION_ENTRY("getClassIData()");
		unsigned short currentStartAddr = m_currentStartAddr % VIRTUAL_ADDRESS_TRANSIT_VALUE;		
		unsigned short currentEndAddr = m_currentEndAddr % VIRTUAL_ADDRESS_TRANSIT_VALUE;
		...
		std::string frameData = m_protocol172->getRemoteMeasurement_C_YC_TA_2(currentStartAddr, currentEndAddr, m_startTime, m_endTime, m_rad);
		ControlStateObserver* pObserver = new ControlStateObserver(m_messageResponseTime);
		pObserver->addObservedState(E_M_0E5H_CON);				// E5
		pObserver->addObservedState(E_M_RESPONSE_DATA_COTD);	// No requested data
		pObserver->addObservedState(CONNECTION_LOST);			// Connection Lost
