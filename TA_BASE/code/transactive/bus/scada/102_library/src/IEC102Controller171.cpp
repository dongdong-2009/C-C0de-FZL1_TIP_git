...
{
	IEC102Controller171::IEC102Controller171(ChannelManager& channelManager,IEC102ConfigData& objConfigData,unsigned short startAddr, unsigned short endAddr)
	:IEC102FlowController(channelManager,objConfigData),
	 m_protocol171(NULL)
	{
		FUNCTION_ENTRY("IEC102Controller171()");
		m_protocol171 = new IEC102Protocol171(*this);
		m_102Protocol = m_protocol171;
	...
	IEC102Controller171::~IEC102Controller171()
	{
		FUNCTION_ENTRY("~IEC102Controller171()");
	...
	bool IEC102Controller171::processFlow()
	{
	...
	void IEC102Controller171::setFlowInfo()
	{
		...
		m_protocol171->setVirtualAddress(virtualAddr);
	...
	bool IEC102Controller171::getNextFlow()
	{
	...
	void IEC102Controller171::getClassIData()
	{
		FUNCTION_ENTRY("getClassIData()");
		std::string frameData = m_protocol171->getClassIData();
		...
		pObserver->addObservedState(E_M_BUFFER_FULL);				// 10 1B 01 00 0C 16
	...
	void IEC102Controller171::call171Data()
	{
		FUNCTION_ENTRY("getClassIData()");
		unsigned short currentStartAddr = m_currentStartAddr % VIRTUAL_ADDRESS_TRANSIT_VALUE;		
		unsigned short currentEndAddr = m_currentEndAddr % VIRTUAL_ADDRESS_TRANSIT_VALUE;
		...
		std::string frameData = m_protocol171->getRemoteMeasurement_C_YC_NA_2(currentStartAddr, currentEndAddr);
		ControlStateObserver* pObserver = new ControlStateObserver(m_messageResponseTime);
		pObserver->addObservedState(E_M_0E5H_CON);				// E5
		pObserver->addObservedState(E_M_RESPONSE_DATA_COTD);	// No requested data
		pObserver->addObservedState(CONNECTION_LOST);			// Connection Lost
