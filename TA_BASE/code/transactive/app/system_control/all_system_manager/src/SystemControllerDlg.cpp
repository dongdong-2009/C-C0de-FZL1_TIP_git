/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/4677_ALL_TIP/TA_BASE/transactive/app/system_control/system_manager/src/SystemControllerDlg.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
//             GetDlgItem(IDOK)->EnableWindow(FALSE);
    ...
    void SystemControllerDlg::populateHostnamesCombo()
    {
        ...
        for ( std::vector<std::string>::iterator itLoop = m_vectorOfHostnames.begin();
              m_vectorOfHostnames.end() != itLoop; ++itLoop )
        {
            m_hostnameCombo.AddString( (*itLoop).c_str() );
        }
    ...
    std::vector<std::string> SystemControllerDlg::getHostnames()
    {
        return m_vectorOfHostnames;
    }


	void SystemControllerDlg::populateHostnames()
	{
		FUNCTION_ENTRY("populateHostnames");

		try
		{
            m_vectorOfHostnames.clear();
            SubSystemControllerAccessFactory::getInstance().getAllSystemControllerNames( m_vectorOfHostnames );
		}
        ...
        if (RunParams::getInstance().isSet("HostnameInclude"))
        {
            std::vector<std::string> result;
            std::set<std::string> includes;
            boost::split(includes, RunParams::getInstance().get("HostnameInclude"), boost::is_any_of(","));

            BOOST_FOREACH(std::string& hostname, m_vectorOfHostnames)
            {
                std::vector<std::string> vs;
                boost::split(vs, hostname, boost::is_any_of("|"));

                if (vs.size())
                {
                    if (includes.find(vs[0]) != includes.end())
                    {
            ...
            m_vectorOfHostnames.swap(result);
        }

        if (RunParams::getInstance().isSet("HostnameExclude"))
        {
            std::vector<std::string> result;
            std::set<std::string> excludes;
            boost::split(excludes, RunParams::getInstance().get("HostnameExclude"), boost::is_any_of(","));
            ...
            BOOST_FOREACH(std::string& hostname, m_vectorOfHostnames)
            {
                ...
                if (vs.size())
                {
                    if (excludes.find(vs[0]) != excludes.end())
                    {
                        continue;
                    }
